    ...
    if not available_tasks:
        ...
        if total_solved >= total_all:
            ...
            keyboard = InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("Нет, я устал", callback_data='tired')]
            ])
            await context.bot.send_message(chat_id=chat_id, 
                text="Вы прорешали все задания! Хотите начать заново?",
            ...
            await context.bot.send_message(chat_id=chat_id, 
                text="Вы прорешали все задания этого типа! Хотите начать заново?",
                reply_markup=keyboard)
        return
...
async def start(update, context):
    ...
    await update.effective_message.reply_text("Выберите тип заданий, которые хотите тренировать:", reply_markup=reply_markup)
...
async def button_callback_handler(update, context):
    ...
    if query.data == 'choose_set_1':
        ...
        if uid in user_ans:
            del user_ans[uid]
        await query.message.delete()
        await context.bot.send_message(chat_id=chat_id, text=f"Начинаем заново! Выбран {display_name}.")
        ...
        await query.message.delete()
        await context.bot.send_message(chat_id=chat_id,
            text=f"Введи ответ ещё раз для задания:\n\n{current_task_text}\n\n✏️ *Напиши ответ:*",
            parse_mode='Markdown')
